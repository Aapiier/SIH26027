...
async function fetchJson<T>(endpoint: string, options?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error(`Failed to connect to RailSync backend at ${API_BASE}: ${err.message || 'Network error'}`);
  }

  const contentType = res.headers.get('content-type') || '';
  if (!contentType.includes('application/json')) {
    throw new Error(
      `Backend at ${API_BASE} returned non-JSON response (${res.status} ${res.statusText}). Please check if the FastAPI server is running.`
    );
  }

  if (!res.ok) {
    const errorBody = await res.json().catch(() => ({ detail: `HTTP ${res.status}: ${res.statusText}` }));
